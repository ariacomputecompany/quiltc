...
impl Client {
    ...
    pub async fn send_json(
        ...
        let _ = self.send_json_bytes(method, path, headers, body).await?;
        Ok(())
    }
    ...
    pub async fn send_json_bytes(
        ...
        loop {
            ...
            if attempt <= self.retries && should_retry(&method, status) {
                let sleep_dur = retry_sleep(status, err.as_ref(), &body_text);
...
fn retry_sleep(status: StatusCode, err: Option<&ErrorResponse>, body_text: &str) -> Duration {
    if status == StatusCode::TOO_MANY_REQUESTS {
        if let Some(e) = err {
